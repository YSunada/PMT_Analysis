...
TH1F* OnePhoto(TTree* works,TTree* dark,int ch)
{
  ...
  //  works    = (TTree*)openfile->Get("TreeOnePhoto_0");
  //dark     = (TTree*)openfile->Get("TreeDark_0");
  ...
  for(int lp_event = 0; lp_event < k_event; lp_event ++)
      ...
      for(int lp_slice = 0; lp_slice < k_num_slice; lp_slice ++)
	{
      ...
      charge[lp_event] = chrg(ch,wform,data_time, wform_cg[lp_event] -5. , wform_cg[lp_event] +5.,data_stopcell);
      dcharge[lp_event] = chrg(ch,dwform,dark_time, dwform_cg[lp_event] -5. , dwform_cg[lp_event] +5.,dark_stopcell);
  ...
  //  openfile->Close();  
  std::cout<<"In OnePhoto_new.h "<<__LINE__<<std::endl;
  return ChargeDist2;
}
