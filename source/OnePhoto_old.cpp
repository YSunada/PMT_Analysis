  ...
  // for(int ch=0;ch<2;ch++)
      ...
      for(int lp=0;lp<15;lp++)
	{
	  ...
	  */
	  ...
	  for(int lp_x=1;lp_x<bin_num2;lp_x++)
	    {
	      two_phe_err[lp_x] =0;	  
	      two_phe[lp_x]     =0;
	      three_phe_err[lp_x] =0;	  
	      three_phe[lp_x]     =0;
	      for(int lp_y=1;lp_y<lp_x;lp_y++)
		{
		  ythree_phe=0;
		  ythree_phe_err=0;
		  for(int lp_z=1;lp_z<lp_y;lp_z++)
		    {		  
		  ...
		  three_phe[lp_x] += (hst_sngl_photo->GetBinContent(lp_x-lp_y))
		    *ythree_phe/(3*Nall); 
		  three_phe_err[lp_x] +=  (hst_sngl_photo->GetBinContent(lp_x-lp_y))
		    *ythree_phe_err;
		  
		  two_phe[lp_x] += (hst_sngl_photo->GetBinContent(lp_x-lp_y))
		    *(hst_sngl_photo->GetBinContent(lp_y))/(2*Nall); 
		  two_phe_err[lp_x] +=  (hst_sngl_photo->GetBinContent(lp_x-lp_y))
